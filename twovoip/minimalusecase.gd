...
func _ready():
	...
	print("Voice chat initialized - mic enabled: ", mic_enabled, " vol enabled: ", vol_enabled)

func add_data(opusdata : PackedByteArray):
	print("Adding audio data to buffer, current buffer size: ", len(opuspacketsbuffer))
...
func _process_record(delta):
	...
	while opuschunked.chunk_available():
		...
		# Skip silence
		if chunkmax < silence_threshold:
			opuschunked.drop_chunk()
			continue
			
		print("Recording audio chunk with max: ", chunkmax)
...
func _process_playback(delta):
	...
	while audiostreamopuschunked.chunk_space_available() and len(opuspacketsbuffer) != 0:
		...
		print("Playing audio packet")

func _on_mic_toggled(enabled: bool) -> void:
	mic_enabled = !enabled
	print("Mic toggled - enabled: ", mic_enabled)
	
	# Clear any pending audio when mic is disabled
	if not mic_enabled:
		while opuschunked.chunk_available():
			opuschunked.drop_chunk()

func _on_vol_toggled(enabled: bool) -> void:
	vol_enabled = !enabled
	print("Volume toggled - enabled: ", vol_enabled)
	
	# Clear buffer when volume is disabled
	if not vol_enabled:
		opuspacketsbuffer.clear()
