...
#######
#var silence_threshold = 0.01  # tweak by testing
...
#func _process_record() -> void:
	#if effect == null or playback == null:
		#return
#
...
#
	...
	#var buf := effect.get_buffer(frames)
#
	## --- Mic toggle ---
	#if not mic_enabled:
		## Still clear the buffer so capture keeps running
		#return  
#
	## --- Silence detection ---
	#var silent := true
	#for sample in buf:
		#if abs(sample) > inputThreshold:
			#silent = false
			#break
#
	#if silent:
...
#
	## --- Store/send audio ---
	#receiveBuffer.append_array(buf)
#
	## --- Playback locally (if volume enabled) ---
	#if volume_enabled:
		#for i in range(min(playback.get_frames_available(), receiveBuffer.size())):
			#playback.push_frame(Vector2(receiveBuffer[i], receiveBuffer[i]))
		#receiveBuffer = receiveBuffer.slice(min(playback.get_frames_available(), receiveBuffer.size()), receiveBuffer.size())
...
#func toggle_mic():
	...
	#print("Mic enabled: ", mic_enabled)
	
#func toggle_vol():
	#vol_enabled = !vol_enabled
	#print("Volume enabled: ", vol_enabled)
#
#func _process_playback(delta):
	#if not vol_enabled:
		#opuspacketsbuffer.clear()  # discard incoming voice
		#return
#
